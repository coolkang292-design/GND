...
### ⚠️ 자산을 갈아 끼웠으면 개발 서버를 반드시 재시작한다

2026-08-07에 **같은 이유로 두 번 헤맸다.** 둘 다 파일은 바뀌어 있었는데 화면이
안 바뀌었다.

```bash
# 자산 교체 뒤 매번
rm -rf .next/dev/cache/images && pnpm dev
```

**① 옛 변환본이 4시간 남는다.** Next 16은 최적화 결과를 `.next/dev/cache/images`에
캐시하고 TTL이 14400초다. 자산을 바꿔도 그 시간 동안 옛 그림이 나온다.
⚠️ 경로가 `.next/cache/images`가 **아니다** — 그쪽을 지우면 아무 일도 안 일어난다.

**② 끊긴 요청이 그 키를 영구히 잠근다.** 최적화기는 같은 키의 동시 요청을
하나로 묶는데, 그 요청이 중간에 끊기면(브라우저 새로고침·curl 타임아웃) 이후
같은 키 요청이 **응답 없이 매달린다.** 그 아이콘만 화면에서 사라진다.
실제로 `hub-routine`·`hub-tabata`가 `w=96`에서만 무응답이었고 `w=32·48·64·128`은
6ms에 나왔다 — **파일이 아니라 캐시 키가 잠긴 것**이다.

### ⚠️ 확인은 앱이 실제로 받는 URL로 한다

`/ui-icons/x.webp`(원본)가 200이어도 **아무 의미 없다.** `<Image>`는
`/_next/image?url=...&w=…&q=75`로 받는다. 폰은 DPR 2라 `w=96`을 받는다.

```bash
curl -H 'Accept: image/webp,*/*' \
  'http://localhost:3000/_next/image?url=%2Fui-icons%2Fhub-part.webp&w=96&q=75'
```

2026-08-07에 원본 경로만 재고 "서빙 확인됐다"고 했다가 틀렸다 — 원본은 새것,
`w=96`은 옛것이었다.
