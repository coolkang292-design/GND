...
**Task 1~13 전부 완료. 전체 게이트 통과(unit 387·실 DB 15/15). 남은 것은 Task 14 = 사용자 실기기 검수 → main 병합 → 배포뿐이다.** 코드는 전부 `feat/xp-level-character-system` 브랜치에만 있고 아직 배포하지 않았다.
...
### Task 14 — 실기기 검수 + 배포 (여기서 이어서)

- [ ] **실기기 검수** — 아래 §7 체크리스트. **사용자 확인 후**에만 다음 단계로.
- [ ] main 병합 → Vercel 배포 → 배포 URL 200 + 캐릭터 이미지 로드 확인.
- [ ] PROGRESS.md의 이번 항목을 🔶 → ✅로 바꾸고 배포 실측치 기록.

### 그 다음 — 0023 계획 완료 보너스

`docs/superpowers/plans/2026-07-23-plan-completion-xp.md` 6개 Task. **0022 배포 후** 시작. 이때 `MAX_DAILY_WORKOUT_XP_NOW`를 **180으로 올리는 것도 같이** 해야 한다(§6).
