...
> 브랜치: `main` (`11f829b`)
> 상태: **홈 경쟁 보드는 배포 완료. 이 문서는 그 뒤에 남은 한 가지 작업이다.**

---

## 1. 다음 에이전트의 단 하나의 다음 행동

`CLAUDE.md` → `PROGRESS.md` → 이 문서를 읽고, **§4의 실패 테스트부터** 시작한다.

할 일은 **하나뿐이다**: 홈에서 사라진 **스트릭 소멸 경고 배너**를 되살린다.
...
- [ ] 어제만 운동한 상태에서 홈에 `⚠️ …(소멸 D-N)` 배너가 뜬다
- [ ] 오늘 운동을 마치면 배너가 **사라진다** (부정 확인)
- [ ] 기록 없음·이미 소멸 상태에서도 배너가 없다
- [ ] 배너 문구를 하드코딩하지 않고 `STAGE_MESSAGES`에서 가져온다
- [ ] 배너 렌더를 지우면 테스트가 **빨개진다** (가짜 통과 점검 완료)
- [ ] 7일 점·헤더 한 줄·옛 `StreakCard`를 되살리지 않았다
- [ ] `나의 오늘` 카드가 배너 포함 330px 이내, 크루 두 행이 하단 탭 위에 남는다
- [ ] lint·typecheck·test·build 전부 통과
- [ ] 사용자 승인 후 배포, 프로덕션 번들에서 `소멸 D-` 확인
- [ ] `PROGRESS.md`와 이 문서를 작업 마지막에 한 번 갱신
