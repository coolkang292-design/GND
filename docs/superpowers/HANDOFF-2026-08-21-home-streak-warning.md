...
- [x] `나의 오늘` 카드가 배너 포함 330px 이내 — **327px**. 지표 칸·배너 여백을 줄여
      되돌렸고, 경고가 없을 때는 **272px**로 개편 전(281px)보다도 작다
...
| 시점 | 단계 | 화면 | 카드 높이 |
|---|---|---|---|
| 실제 오늘 (08-21) | `today_done` | 배너 **없음**, `오늘 완료` 칭찬 배너 | 272px |
| +1일 | `d4` | `⚠️ 11일 연속 중, 오늘 칸만 비어 있어요. 채우면 12일입니다 (소멸 D-4)` | 327px |
| +2일 | `d3` | `⚠️ 11일은 그냥 생긴 게 아니잖아요. 아직 당신 겁니다 (소멸 D-3)` | 327px |
| +3일 | `d2` | `⚠️ 11일 지키실 거면 지금이 마지노선이에요… (소멸 D-2)` | 327px |
| +4일 | `d1` | `⚠️ 마지막입니다. 오늘 안 하면 11일 → 0일… (D-1)` | 327px |
| +5일 | `expired` | 배너 **없음**, 연속 `0일` | 272px |

- 배너는 **지표 3칸 바로 아래, 금색 CTA 위** — 화면에서 네 번째 칸처럼 읽힌다
- 크루 카드 하단 **676px** < 하단 탭 **754px** (경고가 떠도 두 행이 다 보인다)
- 콘솔 오류 **0건** (10회 실행 전부)
- 7일 점·헤더 한 줄은 **없다**
...
2. **크기.** 처음엔 347px로 330px 목표를 17px 넘었다. 사용자 지시
   **"네모 상자를 좀 줄이고 스트릭 칸도 최적화"** 대로 지표 칸의 세로 여백·줄높이
   ...
   `mt-2`, `leading-snug`)을 줄여 **327px**로 되돌렸다.

   ⚠️ 부수 효과가 하나 있다 — 경고가 **없을 때** 카드가 281px → **272px**로 줄었다.
   즉 이 개편으로 정보가 하나 늘었는데 평소 높이는 오히려 9px 작아졌다.
   ⚠️ 여백을 되돌리려거든 **둘을 한 세트로** 봐라. 한쪽만 되돌리면 330px을 넘는다.
   두 곳의 주석(`METRIC_CLASS` · 배너)이 서로를 가리키고 있다.
