...
초대 온보딩 후속 수정의 배포 기준 코드 커밋은 `debc7d5`다(푸시 안 함). **GitHub로 배포하지 않는다.** 검증한 로컬 저장소를 깨끗한 `main`에 합친 뒤 로컬에서 `vercel --prod`로만 배포한다. 배포는 별도 사용자 승인 후 실행한다.
...
**폰 초대 링크는 확인했고 실패했다.** 초대 코드를 자동으로 이어받지 못해 `크루에 들어가요` → `초대 코드로 참여` 화면이 나왔다. 원인은 두 가지였다. ① 모바일에서 `ChallengePage`가 초대 코드를 저장하기 전에 `OnboardingGate`가 `/onboarding`으로 먼저 보낼 수 있었다. ② 첫 수정 뒤에도 `ChallengePage`가 프로필 없는 사용자를 먼저 참가시키고 보관 코드를 지워 일반 온보딩으로 만들었다.

로컬 수정은 다음을 보장하는 테스트를 추가했다.

- `/challenge?join=CODE`를 열자마자 코드를 먼저 기억한다
- 프로필이 없으면 챌린지 화면은 참가를 시도하거나 코드를 지우지 않고 온보딩에 넘긴다
- 새 사용자는 프로필 사진·주간 목표·크루 선택 없이 **닉네임과 `챌린지 참가하기`만** 본다
- 참가 성공 시 코드를 지우고 `/challenge`로 이동한다
- 참가 실패 시 코드를 남기고 같은 화면에서 오류를 보여준다

로컬 Chrome에서 가짜 초대 링크를 열어 `/onboarding` 이동 뒤 **제목 + 닉네임 입력 + `챌린지 참가하기`만** 보이고 일반 크루 선택 요소는 사라진 것을 확인했다. 관련 12/12, 전체 713/713(67파일), lint·typecheck·build를 통과했다.

**남은 확인:** 방장이 새 `setup` 챌린지 링크 생성 → 새 폰/시크릿 창에서 닉네임 입력 → 참가자 등록·목표 설정·주소의 `?join=` 제거를 확인한다. 현재 발급된 `setup` 초대 코드가 없어 실제 성공 참가 1회는 아직 미검증이다.
...
## 8. 2026-07-31 후속 작업 기록 — 초대 온보딩 이후 챌린지 추가 화면

### 8.1 오늘 후속 흐름

1. 사용자 폰에서 챌린지 초대 링크를 열었을 때 일반 크루 선택·초대 코드 입력 화면으로 빠지는 문제를 확인했다.
2. 초대 코드를 온보딩 이동 전에 보관하고, 프로필 없는 사용자는 닉네임 저장 뒤 참가시키도록 수정했다. 초대 사용자는 닉네임과 `챌린지 참가하기` 버튼만 보게 했다.
3. 관련 12/12, 전체 713/713, lint·typecheck·build와 로컬 화면을 확인한 뒤 사용자 승인으로 로컬 저장소에서만 재배포했다. 배포 `gnd-l0hmtp51p-gnd4.vercel.app` → `gnd-one.vercel.app`, 커밋 `debc7d5`. GitHub는 사용하지 않았다.
4. 재배포 뒤 사용자 폰 화면에서 챌린지 추가 동작의 `오류: [object Object]`와 기존 형식으로 미리 채워진 챌린지 이름이 새로 신고됐다.
5. 오류 객체의 실제 `message`를 읽도록 고치고, 추가 버튼·빈 이름 입력 흐름을 단순화했다. 코드 커밋은 `3542973 fix: 챌린지 추가 화면과 오류 안내 개선`이다.
6. 별도 WORKLOG를 만들지 않고 이 최신 인수인계서에 오늘 기록을 합쳤다. 루트 `AGENTS.md`와 `CLAUDE.md`에 다음 에이전트의 시작·검증·종료 규칙을 고정했다.

### 8.2 챌린지 추가 화면 수정의 정확한 내용

- **직접 원인:** Supabase 오류는 `Error`가 아닌 `{ message, code, details, hint }` 형태의 일반 객체일 수 있다. 기존 `String(e)`는 이 객체를 `[object Object]` 한 줄로 뭉개 실제 원인을 숨겼다.
- **오류 안내:** `Error.message` 또는 일반 객체의 문자열 `message`를 읽고 기존 오류 코드별 한글 안내를 그대로 적용한다. 메시지가 없으면 `오류: 알 수 없는 오류`로 표시한다.
- **버튼:** `＋ 챌린지 하나 더 만들기` → `＋ 챌린지 추가하기`.
- **새 이름:** 생성 모드는 빈 문자열, `챌린지 이름을 입력하세요` 안내, 자동 포커스. 목표 수정 모드는 현재 챌린지 이름 유지.
- **범위 밖:** DB, RPC, KPI 계산, 기존 챌린지 데이터는 바꾸지 않았다.

### 8.3 검증 실측과 현재 상태

- 실패 시험을 먼저 확인한 뒤 수정하는 방식으로 오류 2건, 버튼·빈 이름 1건, 안내·포커스 1건을 추가했다.
...
- **폰 확인 완료:** 사용자가 운영 주소에서 챌린지 추가 흐름이 모두 정상 작동한다고 확인했다.

### 8.4 다음 에이전트가 바로 할 일 1개

챌린지 추가 화면 작업은 종료한다. 다음 우선 확인은 아래의 기존 미검증 초대 전 과정이다.
