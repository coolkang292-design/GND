...
**후속 수정 재배포 대기:** 폰 초대 링크에서 일반 크루 온보딩으로 빠지는 문제가 발견됐다. 초대 코드를 온보딩 이동 전에 기억하고, 초대 사용자는 `닉네임 + 챌린지 참가하기`만 보게 하는 수정이 로컬에서 완료됐다. 관련 6/6, 전체 711/711(67파일), lint·typecheck·build를 통과했지만 **현재 운영 주소에는 아직 이 수정이 없다.**
...
**폰 초대 링크는 확인했고 실패했다.** 초대 코드를 자동으로 이어받지 못해 `크루에 들어가요` → `초대 코드로 참여` 화면이 나왔다. 원인은 모바일에서 `ChallengePage`가 초대 코드를 저장하기 전에 `OnboardingGate`가 `/onboarding`으로 먼저 보낼 수 있었기 때문이다.
...
**남은 확인:** 별도 배포 승인 → 검증한 로컬 `main`으로 재배포 → 방장이 새 `setup` 챌린지 링크 생성 → 새 폰/시크릿 창에서 닉네임 입력 → 참가자 등록·목표 설정·주소의 `?join=` 제거를 확인한다. 현재 발급된 `setup` 초대 코드가 없어 실제 성공 참가 1회는 아직 미검증이다.
