...
- [x] **Step 1:** 트랙 3종·조회 실패 테스트 → RED → 구현 → GREEN → 커밋.
...
- [x] **Step 1:** 마이그레이션 작성·커밋(적용은 사용자).
- [x] **Step 2:** 저장·배지 연결, 게이트 통과, 커밋.
...
- [x] **Step 1:** fold/actual 실패 테스트 → RED → 구현 → GREEN → 커밋.

### Task 4: 검증·배포

- [x] **Step 1:** 전체 게이트(unit·typecheck·lint·build) → **사용자 0019 적용** → 실 DB 게이트 재통과(기존 6종 + tabata_minutes insert 확인은 workout-plan-test 관례로 수동 REST 1회).
- [x] **Step 2:** 실기기(8분 코스 블록 전환·자동 기록·피드 배지·챌린지 타바타 목표) → 배포 → PROGRESS 기록.
