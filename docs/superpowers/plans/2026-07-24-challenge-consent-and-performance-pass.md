...
## 1. 결정 사항 — 착수 전 사용자에게 확인 (3건)

계획은 **권장안**으로 작성돼 있다. 사용자가 다르게 정하면 해당 Task의 코드를 조정하라.

**D1 — "5일 연속"의 정의.** 사용자 문구는 "5일 연속". 현재 열람권은 **주(월~일) 5일**(연속 아님)이다.
- **권장(계획 채택):** **엄밀 연속 5일**(오늘 포함 최근 5개 캘린더 날짜 모두 운동). 새 도메인 `hasConsecutiveWorkoutDays(dayKeys, todayKey, 5)`로 판정. "연속"이라는 표현에 충실.
- 대안: 기존 주-5일 유지(코드 변경 최소, 0012 로직 재사용).

**D2 — 열람 대상.** 사용자 문구는 "챌린지 **크루의** 성과"(집합).
- **권장(계획 채택):** **크루 전체 챌린지 순위판**을 공개(개인 1명 선택 아님). `getActiveChallengeRanking(groupId)`의 `list` 전체를 보여준다.
- 대안: 기존처럼 1명 선택 열람 유지.

**D3 — 2시간 창의 의미.** 사용자 문구는 "열람권 사용 시간은 2시간으로 한정".
- **권장(계획 채택):** **5일 연속을 만든 시각부터 2시간** 동안 순위판이 열리고, 지나면 다시 잠긴다(소비 개념 없음, 시간창만). 기존 `record_views` 1회 소비는 이 카드에서는 쓰지 않는다.
- 대안: "처음 연 순간부터 2시간"(소비 기록 필요) 또는 "1회 소비".

> 권장안 3개는 서로 맞물린다: **5일 연속 → 그 시각부터 2h 동안 크루 순위판 공개, 그 외엔 블러+자물쇠, 남은 기간은 항상 표시.**
