...
| **8** | 검증 — A/B 실화면 · 보안 · 회귀 · lint · typecheck · test · build | §5-2 |

**배포는 하지 않는다.**

### 5-2. Phase 8 검증 결과 (2026-09-10)

**자동 테스트** — `typecheck` ✅ · `lint` 0 errors(기존 경고 2건은 무관한 스크립트) ·
`vitest` **3499건 / 206파일 전부 통과**(작업 전 3404 → +95) · `next build` ✅.

**회귀** — `rls-test` **147/147**(129 → +18) · `challenge-photo-test` **8/8** ·
`--tier readonly` **6종 전부 통과**(`default-privilege-check` 21/21로 0096 권한 조임 유지 확인,
`challenge-aggregation-parity` 5/5로 챌린지 집계 무변경 확인).

**⚠️⚠️ 회귀가 진짜 구멍을 잡았다 — `0106`이 그래서 생겼다.**
`challenge-photo-test.mjs`에 예전부터 *"사진 행이 연결된 Storage 파일 삭제는 거부"*라는
단언이 있었다. 0104 이전에는 **DELETE 정책이 아예 없어서** 이 단언이 통과하고 있었고,
0104가 소유자 삭제를 열자 `status=200`으로 깨졌다.
**단언을 고치면 안 되는 것이었다.** `workout_images` 행은 남기고 파일만 지우면,
`get_challenge_period_sessions`가 `exists(workout_images)`만 보므로
**사진 없이 `photo_required` 챌린지 크레딧을 받는다.** 피드엔 깨진 이미지가 뜨지만 집계는 통과한다.
→ `0106`으로 정책을 *"행이 가리키지 않는 객체만"*으로 좁혔다(`alter policy`, drop 없음).
앱의 삭제 순서가 **행 → 파일**이라 정상 흐름은 그대로 동작한다. **그 순서를 뒤집지 마라.**

**데이터 보존** — 마이그레이션 4건과 회귀 5회를 거친 뒤에도
`83행 / 83세션`, 전량 지문 `91310e6bc4a0f73ac70e2181746d8e42` — **작업 시작 전과 동일**.
프로필 8개(CLAUDE.md 기준선 그대로, 회귀 잔여 계정 0).

**고아 객체가 어디서 왔는지 밝혀졌다.** 95개 중 **88개가 `test.jpg`·`priv.jpg`·`slot*.jpg`** —
즉 **`rls-test.mjs`가 과거에 남긴 찌꺼기**다(0104 전에는 지울 수단이 없었다).
이제 스크립트가 자기 뒷정리를 하므로 5회를 돌려도 95개에서 안 늘었다.
남은 88개 정리는 **파괴적 작업이라 승인이 필요하다** — 이번 범위 밖.

**화면 실측 (localhost)** — 캐러셀을 임시 확인용 라우트에 띄워 직접 쟀다(확인 후 삭제):

| 확인 | 결과 |
|---|---|
| 1장·3장·5장 카드 크기 | 전부 341×256, ratio **1.333** — 장수와 높이가 무관 ✅ |
| 1장 카드의 점·카운터·트랙 | **없음** ✅ (부정 확인 — 기존 사용자 화면 무변경) |
| 3장 트랙 | `scrollWidth 1023 = 341×3`, `scroll-snap-type: x mandatory` ✅ |
| scroll 이벤트 | 카운터 `2 / 3` + 점 이동 ✅ |
| 점 클릭 | `3 / 3` 으로 이동 ✅ |
| 사진 탭 | 눌린 장의 index 전달 ✅ |
| 개발 서버 오류 | 0건 · 사진 질의 400 **0건** |

### 5-3. ⛔ 아직 검증 못 한 것 — 사람이 해야 한다

**[미검증] 실제 손가락 스와이프.** 이 세션의 브라우저 패널이 숨겨져 있어
드래그가 30초 타임아웃으로 완료되지 않는다. 스크롤 스냅 CSS·스크롤 핸들러·
점 이동은 실측했지만 **관성 스와이프 자체는 폰이나 보이는 브라우저에서 봐야 한다.**

**[미검증] A/B 두 계정 실화면 전체 흐름** (계획 §7의 시나리오 전량).
로그인이 필요한데 **에이전트가 비밀번호를 입력하지 않는다.**
사용자가 크롬=A(`dev-테스터A`) · 엣지=B로 로그인한 뒤 아래를 봐야 한다:

1. 운동 시작 → 세트 입력 → **상태 배지 옆 📷 버튼**이 보이는가 → 촬영 → `1/5` → 다시 촬영 → `2/5`
2. 촬영 직후 **바로 운동 화면으로 돌아오는가**(기다리게 하지 않는가)
3. 운동 완료 → **썸네일 목록**이 뜨는가 · 추가 · ✕ 삭제 · ◀▶ 순서
4. 마지막 사진 삭제 → *"사진을 모두 지워서 인증이 풀렸어요"* 토스트 · 달력 스탬프가 사라지는가
5. 피드 → **4:3 캐러셀** · 손가락 스와이프 · 점 · `2 / N` · 탭하면 **그 장**이 커지는가 · 더블탭 좋아요
6. B(크루) 화면에서 **A의 사진 전 장**이 넘겨지는가 · 좋아요/댓글 정상
7. 사진 1장짜리 옛 게시물에 **점·카운터가 안 뜨는가**(부정 확인)
