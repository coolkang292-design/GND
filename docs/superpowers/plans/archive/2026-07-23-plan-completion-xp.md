...
**선행 의존성:** `docs/superpowers/plans/archive/2026-07-23-xp-level-character-system.md`(0022 XP 시스템)가 **먼저 구현·배포돼 있어야 한다.** 이 계획은 그 위에 얹는 후속이다. 이 계획의 마이그레이션은 **0023**(0022가 이미 적용된 전제 — 0022 파일은 수정하지 않고 새 마이그레이션으로 분리).
...
-- 설계: docs/superpowers/plans/archive/2026-07-23-plan-completion-xp.md
