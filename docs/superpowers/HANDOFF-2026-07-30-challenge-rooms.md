...
> **2026-07-31 갱신: 0044(전환)는 끝났다.** 적용·배포 완료. 아래 §2.1·§2.3은 **0044 시점의 기록**으로 남겨 둔다(당시 판단 근거). 지금 할 일은 **0045**이고, 상세는 `PROGRESS.md` 최상단 항목과 계획서 `plans/2026-07-30-challenge-rooms-0044.md`의 "0044에서 하지 않는 것" 절에 있다.
>
> **0045 착수 조건은 진행 중 챌린지가 끝난 뒤(2026-09-30 이후)다** — 완료 보너스 `+3 → +9`와 랭킹 RPC 이관이 점수를 바꾸기 때문이다. 0044를 둘로 쪼갠 기준이 "진행 중 챌린지의 점수가 흔들리는가"였고, 흔들리는 쪽을 전부 0045로 미뤘다.
>
> **0045에서 밟기 쉬운 함정 셋** (0044 DB 리뷰가 짚은 것)
> - **정책을 좁힐 때는 `drop policy` + `create policy`가 아니라 `alter policy`를 써라.** 0044는 순수 확대(OR 덧붙이기)라 이름을 틀려도 결과가 같아 무해했다. 0045는 그룹 arm을 **제거**하므로 정반대다 — 이름이 틀리면 `drop`이 조용히 no-op 하고 `0006`의 옛 정책이 살아남아 **그룹 arm이 되살아난다.** `alter policy`는 이름이 틀리면 `42704`로 죽는다(`0014:19`가 선례).
> - **`0039:143-150`의 `select c.id into v_challenge_id ... status='active' ... limit 1`** — 살아있는 챌린지가 여러 개면 **임의의 한 건**이 `record_views.challenge_id`에 박힌다. 0040의 열람권이 이 값을 쓴다. 대표 챌린지 규칙(`pickPrimaryRow`)과 같은 기준으로 바꿔라.
> - **`challenges(group_id)` 대체 인덱스를 만들지 마라.** `challenges_one_live`가 그걸 커버하던 유일한 인덱스였지만 테이블이 1행이고 0045가 그 컬럼을 지운다.
>
> **의도적으로 받아들인 것** — `is_challenge_participant`는 `status`를 안 보므로 `invited`도 그 방의 목표를 읽는다(초대 카드에 필요하고, `0042:77`이 이미 참가자 명단을 열어 뒀다). `dropped`는 행이 영구히 남아 읽기 권한도 영구다(`crew_links` 근거 보존 때문). 둘 다 리크는 아니지만 기록해 둔다.
...
### 6.1b `git push`는 배포가 아니다 — 2026-07-31에 실제로 당했다

이 프로젝트는 **GitHub 연동 배포를 쓰지 않는다.** Vercel이 로컬 저장소에 CLI로 링크돼 있어 `vercel --prod`를 사람이 직접 돌려야 한다.

응원 포인트(7/29 완성)와 챌린지 종목 집계(7/30 완성)를 푸시만 하고 "배포 완료"라고 기록했는데, **이틀간 사용자 폰에는 옛날 코드가 돌았다.** DB는 정상이라 "기능이 왜 안 되냐"의 원인을 찾는 데 오래 걸렸다.

`vercel --prod`는 git 브랜치가 아니라 **현재 폴더 내용**을 올린다. 작업 브랜치에서 그냥 돌리면 미완성 코드가 프로덕션에 나간다. **`git worktree`로 `main`을 분리해서 배포하라.** 상세는 `CLAUDE.md`.
...
| 스크립트 | 이전 | 지금 (2026-07-31, 0044 적용 후) |
|---|---|---|
...
| `scripts/challenge-room-check.mjs` | 32 / 0 | **34 / 0** (0044에서 단언 2개 추가) |
| `scripts/challenge-aggregation-parity.mjs` | (0044 신규) | **4 / 4** |

⚠ **0044가 단언 2개를 뒤집었다.** 둘 다 "두 번째 챌린지가 막힌다"를 정상으로 단언했는데 인덱스를 지웠으니 정반대가 됐다 — `challenge-room-check [21]`(RPC 경로)과 `rls-test:344`(직접 insert 경로). 후자는 **DB 리뷰가 찾았고 계획서가 처음엔 빠뜨렸다**. `pnpm test`에 안 들어가는 스크립트라 게이트가 초록으로 남아 놓치기 쉽다.
