...
새 GND 실행 세션마다 최종 사용자 지정 블랙·골드 배틀로프 원본 이미지를 압축 없이
한 번 보여준다. 개발·운영 실화면 확인과 `main` 반영·운영 배포까지 완료했다.

## 1. 현재 위치와 상태

- 브랜치: `codex/gnd-launch-splash`
- worktree: `C:\Users\SAMSUNG\workout-app\.worktrees\gnd-launch-splash`
- 로컬 `main`: `fcb9799`에 반영
- 운영 배포: `gnd-6849cmciy-gnd4.vercel.app` → `gnd-one.vercel.app`, `READY`
- DB/마이그레이션: 변경 없음
...
사용자가 최종 지정한 이미지 전체를 그대로 사용한다. 별도 HTML 카피는 겹치지 않으며,
Next Image 자동 압축을 끄고 `941×1672` 원본 PNG를 직접 로드한다.

## 3. 주요 파일

| 파일 | 책임 |
|---|---|
| `public/splash/gnd-launch-motivation-v4.png` | GND와 최종 카피가 포함된 사용자 지정 원본 |
...
- 시작 화면 1개, 지정 원본 이미지 `941×1672` 직접 로드, 자동 압축 주소 미사용
- 별도 HTML 카피 0개, 이미지 내 최종 카피만 표시
- 자동 종료 뒤 0개
...
- 배포: `gnd-6849cmciy-gnd4.vercel.app`, 별칭 `gnd-one.vercel.app`, `READY`
- 운영 `/home` 200, `v4` 원본 해시 일치, 이전 `v3` 404
- 운영 브라우저에서 스플래시 1개 → 자동 종료 0개, 원본 `941×1672` 직접 로드
