...
- **⛔ 사람 눈 확인이 남아 있다.** 카카오톡 카드는 **localhost에서 검증이 불가능하다** —
  카카오 스크래퍼가 localhost를 못 긁는다. §4를 보라
- **⛔ 배포하면 2026-09-18 챌린지 탭 개편(0108~0111)도 같이 나간다.** 그쪽은
  "푸시함 · 배포 안 함" 상태였다
...
### ⛔ 확인하지 **못한** 것 — 다음 세션·사용자가 해야 한다

이 세션에는 **브라우저를 조작할 도구가 없었다**(Playwright·chrome MCP 모두 없음).
그래서 아래는 **`[미검증]`이다. "괜찮을 것"으로 넘기지 마라.**

1. **카카오톡 카드 실물** — localhost를 카카오가 못 긁으므로 **프리뷰 배포 후
   "나에게 보내기"** 로만 볼 수 있다. 이 작업의 **최종 증거가 이것이다**
...
개발 서버는 `http://localhost:3000`에 띄워 두었다.
