...
**지금 당장 할 일:** **릴리스 공지 발송**(§5)뿐이다 — 사용자가 지시할 때 사용자가 Run한다.
코드·배포·화면 확인은 전부 끝났다.
...
| 2 | 카카오로 시작 → 돌아옴 | `반가워요!` + 닉네임 칸 하나 | ⚠️ **분기만 확인.** 같은 렌더 분기(`showNicknameStep`)를 8번 경로로 확인했다. 카카오 **왕복 자체**는 §3.3에서 확인됐지만, 신규 가입 경로는 **이제 시험 불가**(§3.3의 경고) |
...
| 5 | 카카오·구글로 연결 | 실제로 연결된다 | ✅ **둘 다 운영 실기기에서 됐다** (2026-08-08, §3.3) |
...
### 3.2 🔴 실제로 이 일이 났다 — 테스트 계정이 사용자의 구글을 물고 있었다

2026-08-08 저녁, 사용자가 구글로 연동하려다 막혔다. `test11`(08-08 가입, 운동 0·XP 0·
챌린지 0·크루 0)이 `coolkang292@gmail.com`을 물고 있었다. 진짜 계정은 `오뎅끼데스까`
(운동 16·XP 1720·챌린지 9개 생성)다.

**신원만 뗄 수 없었다.** 지원되는 경로는 본인 세션으로 부르는
`DELETE /auth/v1/user/identities/{id}` 하나인데(admin 경로는 404 — §2.3),
`test11`은 **구글로만 만들어져 비밀번호가 없다.** 그래서 계정을 지웠다
(사용자 지시). 그룹 `불꽃 크루`(멤버 본인 1명) → auth 계정 순서.
프로필 7 → 6개, 나머지 6개 전부 그대로 확인. **지금 소셜 신원이 붙은 계정은 0개다.**

⚠️ **`listUsers()`는 `identities`를 안 채워 준다** (실측). 전부 빈 배열로 나와서
"아무 데도 안 붙어 있다"고 한 번 잘못 읽었다. **uid마다 `getUserById`를 불러야 한다.**

⚠️ **테스트할 때는 픽스처 계정에 소셜을 붙이지 마라.** 본인 구글·카카오를 한 번
붙이면 그 계정을 지우기 전에는 진짜 계정에 못 붙인다. 앱이 그때 띄우는
`이미 다른 GND 계정에 연결돼 있어요 — 지금 기록은 옮겨지지 않아요`가 바로 이 상황이다.

### 3.3 ✅ 카카오·구글 둘 다 운영 실기기에서 연결됐다 — **배치 3의 핵심 계약이 지켜졌다**

`test11`을 지워 구글을 풀고(§3.2), 카카오 동의항목을 열어(§4.0), 사용자가
`/account`에서 둘 다 눌렀다. 연결 **뒤** DB 실측:

```
오뎅끼데스까 | 운동 16 | XP 1720 | 챌린지 9 | 크루 2
             | email(atty2@naver.com), kakao(atty2@naver.com), google(coolkang292@gmail.com)
```

**한 계정에 신원 셋이 붙었고 숫자가 연결 전과 전부 같다. 새 계정도 안 생겼다**
(프로필 6개 유지). 즉 `linkIdentity`가 제 일을 했다 — §2.1과 §7-1이 경고하는
"`signInWithOAuth`로 갈아타 기록이 갈리는" 실패가 **운영에서 안 났다는 증거**다.
이 표를 지우지 마라. 그 계약이 실제로 지켜진 유일한 실측 기록이다.

⚠️ **§3의 2번(온보딩 첫 화면 → 카카오로 시작 → 신규 가입)은 이제 시험할 수 없다.**
사용자의 카카오·구글이 둘 다 `오뎅끼데스까`에 붙어 있어서, 새 계정으로 다시 누르면
`identity_already_exists`가 정상 동작이다. 세 번째 소셜 계정이 있어야 한다.
**억지로 시험하려고 지금 붙은 것을 떼지 마라** — 얻는 것보다 잃는 것이 크다.
그 화면(모드 2 `반가워요!` + 닉네임 칸)은 초대 링크 경로로 확인했고(§3 8번),
왕복 자체는 위에서 두 제공자 모두 확인됐다.

⚠️ 브라우저가 `/auth/callback`에 떨어졌는지 SITE_URL로 떨어졌는지는 **여전히
[미검증]**이다. Redirect URL 허용목록이 없어도 **연결 자체는 성공한다** —
Supabase가 SITE_URL로 보낼 뿐이다. 화면 흐름까지 보려면 대시보드에서
`https://gnd-one.vercel.app/auth/callback`이 목록에 있는지 확인해야 한다.
...
### 4.0 ✅ 카카오 KOE205 — **해결됐다** (2026-08-08 같은 날)

> 잘못된 요청 (KOE205) · GND 서비스 설정에 오류가 있어, 이용할 수 없습니다.

원인은 **`account_email` 동의항목이 `권한 없음`**이었던 것. 사용자가 카카오 콘솔에서
권한을 열어(`필수 동의`) 그 자리에서 연결에 성공했다. **앱 코드는 한 줄도 안 고쳤다.**

| Supabase가 요청 | 고치기 전 | 고친 뒤 |
|---|---|---|
| `profile_nickname` | 필수 동의 | 필수 동의 |
| `profile_image` | 필수 동의 | 필수 동의 |
| `account_email` | **권한 없음** ← 원인 | **필수 동의** |

⚠️ **`account_email`은 비즈 앱 전환 + 개인정보 동의항목 심사를 거쳐야 열린다.**
사업자등록번호가 없어도 **전화번호 본인인증**으로 비즈 앱 전환이 된다
(디벨로퍼스 → 우측 상단 프로필 → 계정 설정 → 본인인증). 그 뒤 이메일 항목을 따로
신청하고 영업일 3~5일 심사를 받는다. **새 카카오 앱을 만들면 이 절차를 다시 밟아야
한다** — 앱을 갈아엎지 마라.

아래는 그때의 진단 기록이다. 같은 증상이 다시 나면 여기서 시작하라.
...
**같은 증상이 나면 볼 곳** (카카오 개발자 콘솔 → 내 애플리케이션 → GND.
에이전트는 카카오에 로그인하지 않는다): 카카오 로그인 → **동의항목**에서
`닉네임`·`프로필 사진`·`카카오계정(이메일)` 세 개가 전부 사용 설정인지.
**요청하는 scope 중 하나라도 꺼져 있으면 KOE205다.**

급하면 **카카오를 내리는 길**도 있다 — Vercel 환경변수를
`NEXT_PUBLIC_OAUTH_PROVIDERS=google`로 바꾸고 재배포하면 카카오 버튼만 사라진다
(fail-closed 설계가 여기서 쓰인다). 앱 코드는 손댈 것이 없다. **이번엔 안 썼다.**

✅ **카카오·구글 둘 다 운영 실기기에서 연결됐다** (§3.3). 기존 계정에 신원이
추가됐고 기록이 그대로다 — 배치 3의 핵심 계약이 지켜졌다.
