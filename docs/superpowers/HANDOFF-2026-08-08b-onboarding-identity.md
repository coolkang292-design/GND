...
| 항목 | 내용 |
|---|---|
| **홈 주간 목표** | ✅ **끝났다** (아래 §6.1). 커밋 `8122ac8`. 배포는 아직 |
...
**⚠️ 목표가 없을 때 눌리는 칸은 `이번 주 운동` 칸이다** (사용자 지시 —
*"이번주 목표 칸에 목표가 없으면 눌러서 목표세팅하게 해줘"*). 평소 `5 / 5`로
목표가 보이는 자리가 거기라, 없을 때 정하러 가는 문도 같은 자리에 있어야 찾는다.
**옆 `목표 달성률` 칸으로 미루지 마라** — 처음에 그렇게 만들었다가 지적받았다.
`달성률` 칸은 링크가 **아니다**(누를 곳이 둘이면 흐려진다). 단언 2건이 지킨다.

### 6.2 ⚠️ 그 CTA가 막다른 길로 가고 있었다 — 챌린지 화면 게이트

`목표 정하기 ›`를 눌러 `/challenge`에 갔더니 크루 없는 계정에는
**`크루에 참여하면 챌린지를 만들 수 있어요.`** 한 줄뿐이었다. 만들 버튼이 없다.

**0062가 이미 서버를 고쳐 놨는데 화면만 막고 있었다.** `create_challenge_room`은
그룹이 없으면 개인 그룹을 스스로 만든다. 그런데 `challenge/page.tsx`의 게이트 셋이
`group`을 요구하고 있었다 — 만들기 버튼 · 빈 상태 카드 · **설정 시트 자체**.
`handleCreate`는 `group`을 쓰지도 않는다(RPC가 준 `ch.group_id`를 쓴다).

배포된 릴리스 노트는 *"크루가 없어도 챌린지를 만들 수 있어요"*라고 말하는데
화면은 아니라고 하고 있었다. **`group` 조건을 다시 붙이지 마라.**

**화면 확인 (개발 서버, 크루 없는 계정으로 처음부터 끝까지)**

| 단계 | 결과 |
|---|---|
| 목표 없는 홈 | 칸 3개. `0일 / 목표 정하기 ›`가 **`<a href="/challenge">`** · `— / 목표 달성률`(링크 아님) · `0일 / 스트릭`. 세 칸 다 109×72px로 높이가 안 튄다 |
| 페이지 전체 | `/ N` **0건** · `%` **0건** (부정 확인) |
| 칸을 눌렀다 | `/challenge` 도착. `아직 진행 중인 챌린지가 없어요` + `＋ 새 챌린지 만들기` · 옛 문구 `크루에 참여하면` **0건** |
| 시트에서 `계획 운동일 주 6일`로 만들었다 | 크루 없이 챌린지가 **실제로 생겼다**(참가자 1명, 바로 `active`) |
| 홈으로 돌아왔다 | **`0 / 6`** · `0% / 목표 달성률`. 칸이 더 이상 링크가 아니다 |
| 기록 달력 | `0% / 달성률` (목표 생기기 전에는 `— / 목표 미설정`이었다) |

확인용으로 만든 프로필·챌린지·그룹은 전부 지웠다 — 프로필 7개 · 남은 챌린지 0 · 남은 그룹 0.
...
10. **챌린지 화면의 만들기 경로에 `group` 조건을 다시 붙이지 마라** (§6.2).
    0062 이후 서버는 되는데 화면만 막혀 크루 없는 사람에게 막다른 길이 된다
