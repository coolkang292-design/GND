...
**지금 당장 할 일:** ① 🔴 **카카오 동의항목**을 고친다 (§4.0 — 사람이 카카오 콘솔에서)
② 릴리스 공지 발송(§5, 사용자가 Run) ③ 온보딩 첫 화면 카카오 왕복 확인(§3.1, 카카오가 고쳐진 뒤)
...
| 5 | 구글로 연결 | 실제로 연결된다 | ✅ **운영 실기기에서 됐다** (2026-08-08, §3.3) |
...
### 3.3 ✅ 구글 연결이 운영 실기기에서 됐다 — **배치 3의 핵심 계약이 지켜졌다**

`test11`을 지운 뒤 사용자가 `/account`에서 `구글로 계정 지키기`를 눌렀고 성공했다.
연결 **뒤** DB 실측:

```
오뎅끼데스까 | 운동 16 | XP 1720 | 챌린지 9 | 크루 2
             | email(atty2@naver.com), google(coolkang292@gmail.com)
```

**같은 계정에 신원이 하나 더 붙었고 숫자가 연결 전과 전부 같다. 새 계정도 안 생겼다**
...
⚠️ 다만 브라우저가 `/auth/callback`에 떨어졌는지 SITE_URL로 떨어졌는지는 **여전히
...
✅ **구글은 운영 실기기에서 연결됐다** (§3.3). 기존 계정에 신원이 추가됐고 기록이
그대로다 — 배치 3의 핵심 계약이 지켜졌다. 카카오만 위 KOE205로 막혀 있다.
