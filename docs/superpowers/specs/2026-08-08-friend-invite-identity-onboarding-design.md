...
[ 닉네임만으로 시작하기       → ]     ← 부 버튼(테두리)
   이 브라우저에만 저장돼요. 나중에 내 정보에서 연결할 수 있어요.
이미 계정이 있나요? 로그인
```
...
|---|---|---|---|
| 1 | **D** 단계 알약 | 없음 | 없음 |
| 2 | **A** 친구 초대 링크 + **챌린지 신입 자동 연결**(§3.6) | `0061` · `0062` · `0063` | 사용자가 SQL Editor에서 Run · 픽스처 A·B 화면 확인 |
| 3 | **C** 카카오·구글 배관 + `/account` + `/login` | 없음 | §5.3 사용자 설정 7단계 |
| 4 | **B** 온보딩 개편 + 자산 2장 + **프로필 편집 시트**(§4.3) | 없음 | 배치 3 · 사람이 리다이렉트 왕복 확인 |
