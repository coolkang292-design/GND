...
**포인트 내역 화면은 없다.** `getRecentPointTransactions`(`points.ts:31`)는 정의만 있고 **호출부가 0건인 죽은 코드**다. 화면에 붙어 있는 것은 `PointSummary`(잔액·배수·불꽃)뿐이다(`growth-hub.tsx:155`). 따라서 `cheer_sent` 라벨을 추가할 대상 화면이 존재하지 않으므로 **이번 범위에서 제외한다.** 내역 화면을 새로 만드는 것은 이 기능의 목적(지급 인식)에 필요하지 않다 — 토스트가 그 역할을 한다.
...
- 이모지 리액션 포인트 (D4)
- 받는 사람 지급 (D3)
- **포인트 내역 화면** — `getRecentPointTransactions`가 죽은 코드로 남아 있으나(§4.4) 이번에 화면을 만들지 않는다
- 응원 관련 배지 (예: "응원왕 100회")
