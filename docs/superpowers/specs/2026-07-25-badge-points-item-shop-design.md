...
**아이템은 전부 패션과 성공의 상징으로 구성한다.** 명품 의류·신발·가방·시계와
슈퍼카·펜트하우스·전용기다. 운동용품(프로틴·마사지건·바벨)은 넣지 않는다 —
운동은 이미 앱에서 실제로 하고 있으므로 그 도구를 사는 건 대리만족이 안 된다.
사용자가 얻으려는 건 **운동의 결과로 도달한 삶**의 감각이다.
...
| D10 | `user_equipment`·`user_items`는 **크루원 select 허용** | 다른 테이블은 전부 본인 전용이나, 자랑이 목적이므로 의도된 예외 |
| D11 | 아바타는 **중성 1세트**, 성별 구분 없음 | 캐릭터가 불독이라 성별 표현이 스테레오타입이 된다. 나누면 아이템도 2배(29→58장)가 되고 추가 비용이 영구적으로 2배. 성별 표현은 아이템 선택에 맡긴다 |
| D12 | 아바타 기본 복장은 **밀착 검은 베이스 레이어**, 7장 동일 | 옷 위에 옷을 덮는 구조라 기본 복장이 두꺼우면 아이템 밑에서 삐져나온다 |
| D13 | 7장의 차이는 **표정뿐**. 단계 오라는 **CSS glow**로 스택 최상단에 얹는다 | 몸통 털·팔 오라를 이미지에 구우면 상의를 입는 순간 덮인다. CSS로 빼면 옷 바깥 실루엣을 따라 살아나고, 목 아래가 7장 동일해져 아이템 호환이 보장된다 |
...
items
  item_key     text pk
  name, description
  slot         text  -- head|top|bottom|shoes|wrist|bag|hand|prop
  category     text  -- 표시용 분류 (의류·신발·가방·시계·차량·부동산…)
...
| z | 슬롯 |
|---:|---|
| 10 | `prop` (배경형 — 펜트하우스·전용기) |
| 20 | `bag` (백팩 — 어깨 뒤로 넘어가는 것) |
...
| 70 | `wrist` |
| 80 | `head` |
| 90 | `hand` (손에 든 것 — 샴페인·서류가방·클러치) |
| 100 | `prop` (전경형 — 차량) |
| 200 | **단계 오라** — CSS `drop-shadow` glow (이미지 아님) |

표의 z는 슬롯 **기본값**이고, 실제 순서는 `items.z_layer` 행 값이 결정한다.
같은 슬롯이라도 앞뒤가 갈리는 경우가 있기 때문이다:

- `prop` — 펜트하우스·전용기는 배경(10), 차량은 전경(100)
- `bag` — 백팩은 아바타 뒤(20), 핸드백·토트는 앞(85)

구현은 `position:absolute` 스택 — 캔버스 조작·이미지 라이브러리 불필요.
...
전 종목이 **패션 아니면 성공의 상징**이다. 운동용품은 넣지 않는다(§1).
등급이 오를수록 "좋은 운동복 → 명품 → 슈퍼카 → 부동산·전용기"로,
현실에서 성공이 눈에 보이는 순서 그대로 올라간다.

### 🥉 common — 첫 주 ~ 첫 달 · 좋은 운동복
...
| 👖 리바개 501 | bottom | 1,300P | 15만 |
| 🕶️ 레이개 선글라스 | head | 1,500P | 25만 |

### 🥈 rare — 2 ~ 8개월 · 남이 알아보는 것

| 아이템 | 슬롯 | 가격 | 실제가 |
|---|---|---:|---|
| 🧥 아디다개 트랙탑 | top | 2,500P | 20만 |
| 🥂 개페리뇽 샴페인 | hand | 3,000P | 40만 |
| 🎩 버버개 체크 버킷햇 | head | 3,200P | 60만 |
| 👟 개조던 1 레트로 | shoes | 3,500P | 40만 |
| ⌚ 애플워개 울트라 | wrist | 4,000P | 120만 |
| 🧥 노개페이스 눕시 | top | 4,500P | 50만 |
| 🎒 루이비개 백팩 | bag(뒤) | 5,500P | 350만 |
| 👟 발렌시개 트리플S | shoes | 7,000P | 130만 |

### 🥇 epic — 1 ~ 2년 · 명품

| 아이템 | 슬롯 | 가격 | 실제가 |
|---|---|---:|---|
| 👞 구찌개 홀스빗 로퍼 | shoes | 9,000P | 150만 |
| 🧥 몽클개 패딩 | top | 10,000P | 300만 |
| 💼 루이비개 서류가방 | hand | 12,000P | 500만 |
| 🧥 버버개 트렌치코트 | top | 14,000P | 350만 |
| 👜 개넬 클래식 플랩백 | bag(앞) | 16,000P | 1,500만 |
...
| 아이템 | 슬롯 | 가격 | 실제가 | 도달 |
|---|---|---:|---|---|
| 🚗 테슬개 모델 개 | prop(앞) | 28,000P | 8,000만 | 약 8개월 |
| 👜 에르개스 버킨 | bag(앞) | 35,000P | 5,000만 | 약 10개월 |
...
| 시각 | 아바타 7장 × 8슬롯 풀장착 합성 스크린샷 육안 확인 — 특히 기본 베이스 레이어가 아이템 밑에서 삐져나오지 않는지 |
...
- 기존 성장 캐릭터 7장 재제작
- **운동용품 아이템** (프로틴·마사지건·바벨·러닝머신) — §1 참조
