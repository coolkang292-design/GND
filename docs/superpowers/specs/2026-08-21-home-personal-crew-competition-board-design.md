...
닉네임·레벨 진행 영역이 누를 수 있는 자리이고, CTA는 별도 형제로 둔다.

⚠️ **`/profile`(설정)로 보내지 않는다** (2026-08-21 사용자 지시 E — "내 프로필을
클릭하니까 설정 화면으로 랜딩되네, 다른 크루와 동일한 화면으로 표시되게"). 크루 행은
누르면 `MemberProfileSheet`가 열리는데 내 행만 설정으로 가면 **같은 자리에서 같은
모양을 누른 결과가 사람마다 달랐다.** 이제 홈의 세 행 모두 같은 시트를 연다.

- `get_crew_member_profile`은 본인을 허용한다 — `p_target_id <> auth.uid()`일 때만
  크루를 따진다(`db-current-schema.sql:1255`).
- 접근 가능한 이름도 크루 행과 같은 `성과 보기`를 쓴다.
- 시트는 **홈이 소유한다.** 카드는 조회도 상태도 갖지 않는다.
- `viewerId`·`source`는 **넘기지 않는다.** 내가 내 것을 연 것은 프로필 방문이 아니라
  크루 방문 통계를 자기 조회로 부풀린다.
- 설정 화면은 하단 탭 `내 정보`가 그대로 맡는다.
