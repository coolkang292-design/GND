...
목업은 방향과 정보 위계를 확정하기 위한 것이다. 실제 구현은 기존 GND 토큰·아이콘·
컴포넌트를 사용하며, 375px와 390px 화면에서 글자 잘림과 접힘선을 다시 측정한다.
...
- 이번 주 운동일: 목표가 있으면 `1 / 5`, 없으면 `1일 · 목표 정하기 ›`
- 현재 스트릭: 예) `10일`
- 보유 배지 수
- 사실 중심 비교 문구: 예) `크루 2명 중 1명 완료 · 나는 아직`
- 상태별 주 행동 영역

비교 문구의 뒷부분은 내 상태에 맞춰 `나는 아직` / `나는 운동 중` / `나도 완료`로
바꾼다. 완료 인원은 크루만 세고 내 완료를 분자에 섞지 않는다.
...
- 제목: `오늘의 크루`
- 요약: `완료 인원 / 전체 크루 인원`, 예) `1 / 2명 완료`
- 접힌 표시 수는 최대 2명이다. 내 카드와 크루 두 행을 첫 화면에 함께 보여주기 위한
  새 기준이다.
- 2명을 넘으면 `전체 보기 ›`를 표시한다.

요약은 죄책감이나 도발 문구 없이 실제 완료 인원만 말한다.
...
홈 행에서는 누적 운동 횟수, 누적 시간, 배지 썸네일을 제거한다. 이 정보는 오늘의
...
- 배지 수: 기존 배지 카탈로그와 `earnedBadgeCount` 규칙을 공유한다.
...
1. 375×812와 390×844에서 내 카드와 크루 두 행이 첫 화면에서 함께 읽힌다.
