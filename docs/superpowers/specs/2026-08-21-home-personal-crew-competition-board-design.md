...
아래 네 가지를 최종 보완안으로 승인했으며, **이미지보다 이 문서 규칙이 우선한다.**

1. 내 카드의 배지 타일 제거
2. 크루 헤더의 `1 / 2명 완료` 중복 칩 제거
3. 운동 전 비활성 `콕`의 이유를 한 줄로 표시
4. 내 카드·크루 행에 실제 높이 제한 적용
...
완료 인원 요약은 내 카드의 비교 문구에서 한 번만 말한다. 크루 헤더에 `1 / 2명 완료`
칩을 반복하지 않는다.
