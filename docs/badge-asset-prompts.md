...
> **✅ 2026-07-27 제작 완료.** 30장 전부 `public/badges/`에 있다.
> 아래 §4에 실제로 통한 방법과 걸렸던 함정을 적어뒀다 — 배지를 추가할 때 읽을 것.
...
## 4. 실제 제작 기록 (2026-07-27) — 다음에도 이렇게 한다

낱장 30번이 아니라 **티어별 시트 4장**으로 만들고 스크립트로 잘랐다. 한 시트 안에서
프레임이 자동으로 일치해서, 낱장으로 뽑는 것보다 결과가 고르다.

| 시트 | 티어 | 배치 | 장수 |
|---|---|---|---:|
| 1 | 🥉 브론즈 + 🔥반복 | 3×3 | 9 |
| 2 | 🥈 실버 | 3×3 | 9 |
| 3 | 🥇 골드 | 3×3 (마지막 칸은 빈 배지로 나옴 → 버림) | 7 |
| 4 | 👑 레전드 | 3×2 | 5 |

원본은 1536×1024, 장당 코어 260~330px. 잘라서 **384×384**로 정규화했고 장당 49~63KB다.

### 배치 순서

시트 안에서 **왼쪽 위 → 오른쪽 → 다음 줄** 순으로, 이 문서 §2의 표 순서를 그대로
따랐다. 스크립트(`scripts/slice-badge-sheets.mjs`)의 `MAP`이 이 순서를 그대로 갖고
있으므로, 시트를 다시 만들 때도 같은 순서를 지켜야 한다.

### 걸렸던 함정 세 가지

**① 세로 간격이 너무 좁았다.** 1차 결과에서 위아래 배지의 꼭짓점이 딸려 들어왔다.
가로 간격은 137px인데 세로가 31px이었다. 스크립트가 이웃 중심까지의 거리로 축별
여백을 잘라 막지만, **애초에 세로도 가로만큼 띄우는 게 낫다.**

**② 배지 크기가 들쭉날쭉했다.** 이웃 간격에 따라 잘린 여백이 달라져서, 나란히 놓으면
크기가 달라 보였다. 지금은 **코어 바운딩 박스가 캔버스의 74%를 차지하도록 정규화**한다
(`CORE_FRAC`). 나머지 26%는 글로우 여백이다.

**③ 투명 배경은 눈으로 판단하면 안 된다.** 뷰어가 반투명 글로우를 배경색과 합성해서
보여주기 때문에, 갈색 배경이 칠해진 것처럼 보였다. 실제로는 모서리 알파가 0이었다.
**알파 채널을 직접 찍어봐야 안다.**

### 검수 방법

30장을 하나씩 열지 말고 **어두운 배경 대지에 모아 붙여** 한 번에 본다.
실제로 얹힐 배경(앱 카드는 `#121214` 근처)에서 봐야 글로우와 테두리가 제대로 보인다.
