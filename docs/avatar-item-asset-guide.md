...
```
몸 중심선     가로 50% (512px)
어깨 너비     캔버스 폭의 48% (492px)
```

머리가 전체 키의 약 1/4을 차지하는 **다부진 3~4등신**이라 어깨선이 낮고
어깨 너비가 넓다. 사람 8등신 비율이 아니다.
...
7장의 유일한 차이는 **얼굴 표정**이다. 털 상태나 오라를 단계별로 그려 넣지 않는다.

- **옷에 가려진다.** 몸통 털이나 팔 주위 오라를 아바타에 그려도 상의를 입는
  순간 덮인다. 옷에 안 가려지는 건 얼굴뿐이다.
- **아이템 호환성.** 표정만 바꾸면 목 아래가 7장 모두 픽셀 단위로 동일해져
  어느 단계에서든 아이템이 정확히 맞는다.

| 단계 | 표정 |
|---|---|
| 1 개노답 | 지쳐 풀 죽음 — 눈 반쯤 감김, 눈썹 처짐, 입꼬리 내려감, 시선 아래 |
| 2 눈떴개 | 막 깨어남 — 한쪽 눈썹 치켜올림, 눈 크게 뜸, 입 살짝 벌림 |
| 3 일단하개 | 담담한 각오 — 정면 응시, 눈썹 수평, 입 다뭄 **(기준)** |
| 4 물고가개 | 악착같음 — 눈 가늘게, 눈썹 모임, 이 악물고 턱에 힘 |
| 5 미쳐보개 | 광기 — 눈 부릅뜸, 흰자 보임, 이 드러낸 사나운 웃음 |
| 6 판을짜개 | 계산 — 실눈, 한쪽 눈썹만 올림, 한쪽 입꼬리만 올린 미소 |
| 7 전설이개 | 관조 — 눈 반쯤 감은 여유, 눈썹 편안, 미세한 미소, 턱 살짝 들림 |

**단계 오라는 이미지가 아니라 CSS로 처리한다** (§4 z-순서 참조). 합성 스택
최상단에 glow 레이어를 얹으면, 패딩을 입어도 오라가 옷 바깥 실루엣을 따라 살아난다.
아바타에 구우면 소매에 덮여 사라진다.
...
| **200** | **단계 오라** | CSS glow — 이미지 아님 |

### 단계 오라 (z=200, CSS)

이미지로 만들지 않는다. 합성 스택 **최상단**에 CSS `drop-shadow`를 얹어
전체 실루엣에 빛을 두른다. 옷을 입어도 옷 바깥 테두리를 따라 살아난다.
