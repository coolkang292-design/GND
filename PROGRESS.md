...
## ⏸️ 2026-08-21 배포 대기 — 홈 스트릭 소멸 경고 복원

홈 개편에서 `StreakCard`를 빼면서 같이 사라졌던 **소멸 경고 배너**(`⚠️ … 소멸 D-N`)를
`PersonalTodayCard` 안, **`이번 주 · 연속 · 배지` 3칸 바로 아래**(금색 CTA 위)에
되살렸다. 되살린 것은 **경고 하나뿐**이다 — 7일 점·헤더 한 줄·옛 카드는 사용자가
고르지 않았고, `home-client.order.test.ts`가 그 부재를 계속 단언한다.

- 코드: `src/components/home/personal-today-card.tsx` (+테스트 5건)
- 판정은 옛 `home/streak-card.tsx:44-47`을 **그대로** 옮겼다. 문구는 전부
  `STAGE_MESSAGES`에서 온다 — 이 카드가 문장을 짓지 않는다
- DB·RPC·마이그레이션: **없음**
- 게이트: lint 0 error(경고 2건은 미추적 `scripts/make-study-pack.mjs`) · typecheck ·
  test 2465건 · build 전부 통과
- **개발 서버 화면 확인 완료** (375×812, `dev-테스터A`, 직접 로그인해 조작):

| 상태 | 화면 | 카드 높이 |
|---|---|---|
| 오늘 완료(실제 오늘) | 7일 점만, 배너 **없음** | **302px** |
| d4·d3·d2·d1 | 7일 점 + 주황 `⚠️ … (소멸 D-N)` 칸 | **357px** |
| 소멸(5일 경과) | 배너 **없음**, 연속 `0일` | **302px** |

  크루 두 행은 다 떠도 하단 탭 위(706px < 754px). 콘솔 오류 0건.
- **사용자가 화면을 보고 세 번 고쳤다**: ① 자리를 CTA 아래 → 지표 3칸 바로 아래로
  ② "네모 상자를 좀 줄이고 스트릭 칸도 최적화" → 지표 칸·배너 여백을 줄여 347px을
  327px로 ③ 그렇게 생긴 자리에 **최근 7일 점**을 넣으라고 지시
- ⚠️ 최종 높이는 330px 목표를 넘는다(302/357px). 목적인 "크루 두 행이 탭 위"는
  지켜지고(706 < 754), **사용자가 그 맞바꿈을 알고 골랐다**
- ⚠️ 여백을 되돌리려면 지표 칸과 배너를 **한 세트로** 봐라 — 주석이 서로를 가리킨다
- 7일 점의 창은 `weeklyBars`(기록 화면과 같은 함수)가 정한다 — 홈이 따로 세지 않는다
- 뜨는 쪽을 보려고 **운영 기록을 건드리지 않았다.** 브라우저 시계만 하루씩 앞으로
  돌려 A의 마지막 운동일(08-21)을 어제~5일 전으로 만들었다(홈은 읽기 전용)
- **미배포.** 사용자 승인 후 `CLAUDE.md` 절차대로 배포한다
- 상세·함정: `docs/superpowers/HANDOFF-2026-08-21-home-streak-warning.md`
