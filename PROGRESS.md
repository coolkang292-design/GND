...
- **A가 오늘 운동을 마친 뒤의 상태** — 칭찬 배너, 금색 콕 활성, `✅ 찌름` 전이.
  계획서가 "검증하자고 운영 운동 데이터를 만들지 말라"고 못 박아 두어 멈췄다.
  컴포넌트 테스트는 셋 다 덮고 있지만 화면은 못 봤다.
- **B 계정 화면과 알림 벨 수신** — 로그인에 비밀번호 입력이 필요해 에이전트가 하지
  않았다. 사용자 확인 필요.
