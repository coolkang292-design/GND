...
**Phase 0·1·2·3 완료(코드). ⚠️ 0004 마이그레이션 적용 + RLS 검증 대기.**
...
| 3 운동 핵심 | 🔶 코드 완료 | 세션·RPC·카탈로그·세트입력·휴식타이머·임시저장. **0004 수동 적용 후 `node scripts/rls-test.mjs` 필요** |
| 4~7 | 대기 | 계획서 §18 참조 |
...
- `scripts/rls-test.mjs` 15 → 35케이스 확장(카탈로그 격리·상태전이·컬럼 권한·크루 공개/비공개 경계).
...
**0004은 미적용 — 적용 후 `node scripts/rls-test.mjs` 실행할 것.**
...
1. **사용자: 0004 적용** (SQL Editor) → `node scripts/rls-test.mjs` 35케이스 확인
2. 실기기 스모크: 운동 추가→시작→세트 완료(휴식 타이머)→종료→완료 화면, 새로고침 복구
3. Phase 4 착수 (완료 루프): 사진 압축·업로드·달력 스탬프(계산)·날짜상세·지난 운동 복사 — `calendar.ts` tz TDD
