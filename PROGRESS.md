...
## ⏳ 2026-08-17 개발 완료 — GND 실행 동기부여 스플래시 (배포 안 함)

브랜치 `codex/gnd-launch-splash`, 격리 worktree
`C:\Users\SAMSUNG\workout-app\.worktrees\gnd-launch-splash`에서 구현했다.
**로컬 `main` 미반영 · 운영 배포 미실행 · DB/마이그레이션 변경 없음.**

### 사용자 화면

- 일반 탭(`/home`, `/feed`, `/record`, `/challenge`, `/profile`)의 새 브라우저 실행
  세션에서 시작 화면을 **1회** 표시한다.
- 비·수증기·젖은 바닥·배틀로프 성인 남녀·블랙/골드 조명의 승인 배경과 상단 `GND`를
  표시한다.
- 생성 이미지의 한글 오타(`같은` → `갈은`)를 근본적으로 피하려고 배경에서 하단
  문구를 제거하고 실제 HTML 텍스트로 아래 두 줄을 렌더한다.
...
- 첫 줄 흰색, 둘째 줄 골드, 굵은 전진 기울기와 왼쪽 속도선을 적용했다.
- 이미지 준비 후 1.5초 뒤 자동 종료하며 화면 터치 시 즉시 건너뛴다.
- 같은 세션의 새로고침·탭 이동에는 다시 보이지 않고, 새 세션에는 다시 표시한다.
- 승인 배경: `public/splash/gnd-launch-motivation-v3.png`, SHA256
  `0092B12118DD9ED60B7AC985E0829BAAA86346CCA737BA75BC8D2D92B08357B5`,
  2,511,843바이트. 원본과 앱 자산 해시가 같다.

### 개발 서버 실조작

`http://localhost:3011`의 실제 브라우저 화면에서 확인했다.

- 시작 오버레이 1개, 배경 이미지 실제 로드 `430×763`, 최적화 요청 폭 `640px`
- `지금은 같은 출발선.` 정확한 요소 1개, `갈은 출발선` 0개, 둘째 줄 1개
- 약 1.5초 뒤 오버레이 0개
...
- **[미검증] 실제 휴대폰 설치형 PWA를 완전히 종료한 뒤 다시 실행하면 스플래시가
  다시 1회 보이는지 확인하지 못했다.** 브라우저 백그라운드 복귀는 같은 세션으로
  간주해 재표시하지 않는 설계다.
- 다음 할 일 1개: 휴대폰 개발 서버에서 완전 종료/재실행을 사용자가 확인한 뒤,
  로컬 `main` 반영과 운영 배포 승인을 받아 Vercel CLI 배포·프로덕션 실물 확인.
