...
## 🧢 2026-08-01 — 좌표 기반 캐릭터 아이템 목업 (개발 서버 확인 ✅ · 운영 배포 안 함)

- 사용자가 제공한 새 불독 이미지를 기준으로 장비가 없는 1024×1536 RGBA 기본 캐릭터를 생성했다. 검은 민소매·검은 짧은 타이츠·맨발 상태이며 원본과 생성 자산은 `docs/design-sources/avatar-coordinate-v2/`, 앱 자산은 `public/avatar-coordinate-v2/`에 있다.
- 머리·귀·눈·코·목·어깨·허리·손목·발 16개 점과 7개 장착 영역을 `landmarks.json`에 고정했다. 아이템은 완성 사진 교체가 아니라 잘라낸 투명 PNG와 `x/y/width/height/z`를 1024×1536 기준 백분율로 바꿔 실시간 합성한다.
- 모자, 선글라스, 후드, 조거팬츠, 하이탑, 손목시계 6개를 독립 PNG로 만들었다. 현재 실제 장착 검증 대상은 `gnd-cap-v2` 하나이고 나머지 5개는 화면에 `준비 중`으로 표시해 장착할 수 없다. 의류 5개는 체형 맞춤 보정 전이므로 운영 품질 완료로 간주하지 않는다.
- `/profile`의 포인트 요약 바로 아래에 `캐릭터 아이템 상점` 카드 1개를 추가했고 `/profile/avatar-mock`으로 연결했다. 목업 잔액 12,840P에서 모자 500P 구매 → 12,340P → 장착 → 해제 → 재장착이 가능하며 새로고침하면 초기화된다. 실제 포인트·DB는 건드리지 않는다.
- 개발 서버 `localhost:3107`에서 직접 확인: 프로필 진입 카드 1개, 준비 중 버튼 5개, 구매 후 잔액, 장착 레이어 1개, 해제 후 0개, 재장착 후 1개, 새로고침 초기화. 320/390/430px에서 미리보기 2:3 비율과 모자 좌표 비율이 유지되고 가로 넘침이 없었다. 브라우저 콘솔 오류 0건.
- 최종 리뷰 반영: 앱과 검증기가 `avatar-coordinate-manifest.json` 한 장을 좌표 원본으로 공유한다. 6개 상품 썸네일을 192×192 WebP로 파생해 합계 약 30KB로 줄였고 Next 이미지 최적화를 켰다. 운영 빌드는 `NEXT_PUBLIC_ENABLE_AVATAR_MOCK=true`가 없으면 진입 카드를 숨기고 `/profile/avatar-mock`을 404로 막는다. 로컬 프로덕션 서버에서 `/profile` 200, 목업 경로 404를 확인했다.
- 자산 검사: 기본 캐릭터와 6개 아이템 모두 RGBA, 투명 모서리, 실제 크기, 캔버스 경계 통과. 흰색·어두운 배경 QA와 모자 단독 QA를 생성했다.
- 검증: 신규 관련 16/16, 전체 unit **861/861(80파일)**, lint 0, typecheck, build, 자산 검사 통과. 개발 빌드에는 `/profile/avatar-mock` 정적 경로가 포함되고 운영 실행에서는 플래그가 없으면 404다.
- **미포함:** 실제 포인트 차감, 보유 아이템 DB, 로그인 간 장착 저장, 운영 배포. 이 기능들은 별도 DB·권한 설계와 사용자 승인 후 진행한다.
