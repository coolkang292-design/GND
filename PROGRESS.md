...
### 🔴 아직 남은 같은 원인 — 다음 사람이 이어서 할 것

**기존 만들기 폼(`openSheet("create")`)의 기본 시작일이 아직 `오늘`이다**
(`challenge/page.tsx`). 그대로 저장하면 다음 화면 조회가 방을 시작시켜
**초대가 즉시 닫힌다** — 위의 참가자 1명짜리 14개를 만든 바로 그 경로다.
이번 배포에서는 **건드리지 않았다**(승인 범위 밖). **[미검증]**
