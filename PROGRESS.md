...
| 상태 | 화면 | 카드 높이 |
|---|---|---|
| 오늘 완료(실제 오늘) | 배너 **없음** | **272px** |
| d4·d3·d2·d1 | 주황 `⚠️ … (소멸 D-N)` 칸, 지표 3칸 **바로 아래** | **327px** |
| 소멸(5일 경과) | 배너 **없음**, 연속 `0일` | **272px** |

  크루 두 행은 경고가 떠도 하단 탭 위(676px < 754px). 콘솔 오류 0건.
- **사용자가 화면을 보고 두 번 고쳤다**: ① 자리를 CTA 아래 → 지표 3칸 바로 아래로
  ② "네모 상자를 좀 줄이고 스트릭 칸도 최적화" → 지표 칸·배너 여백을 줄여 347px을
  **327px**로 되돌렸다. 그 덕에 경고가 없을 때는 개편 전(281px)보다 9px 작다
- ⚠️ 여백을 되돌리려면 지표 칸과 배너를 **한 세트로** 봐라 — 한쪽만 되돌리면 330px 초과
