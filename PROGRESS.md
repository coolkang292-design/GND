...
- **⚠️ 접근에 환경변수가 필수** — `ADMIN_USER_IDS`(쉼표 구분 uuid, Vercel Production에 등록 완료 = 오뎅끼데스까). **비어 있으면 관리자 본인도 404다(fail-closed)** — 설정 누락이 전면 개방으로 이어지면 안 되기 때문. 거부는 403이 아니라 **404**로 준다(403이면 페이지 존재가 드러난다). 게이트는 `requireAdmin()` **한 곳뿐**이고 **RLS 정책은 한 줄도 안 건드렸다** — 8개 테이블에 관리자 select 정책을 뿌리면 하나만 느슨해도 사용자 앱에서 남의 데이터가 샌다.
...
- **남은 것 — 폰 확인 1건**: 익명 인증이라 오뎅끼데스까 세션이 폰에만 있어 **로컬에서는 관리자 화면을 열 수 없다**. 폰에서 `gnd-one.vercel.app/admin` 접속 → 대시보드가 뜨는지 + **표의 스트릭·레벨이 홈 🔥·내 정보와 같은지** 대조가 남았다.
