...
## 🟡 2026-08-17 작업 — 관리자 대시보드 공백 지표 (브랜치, **미배포**)

브랜치 `feat/admin-gap-metrics` 커밋 2개. **배포 안 했다 — 사용자 승인 대기.**
...
- **[미검증]** 프로덕션 실물 — 아직 배포하지 않았다
- **[미검증]** 신규 등록의 기간 분리는 화면에서 못 갈랐다 — 등록 17건이 전부 최근
  7일 안이라 7/28/90이 모두 17이다. 규칙 자체는 단위 테스트가 고정한다
