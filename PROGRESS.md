...
`PersonalTodayCard` **안**, 금색 CTA 아래에 되살렸다. 되살린 것은 **경고 하나뿐**이다 —
7일 점·헤더 한 줄·옛 카드는 사용자가 고르지 않았고, `home-client.order.test.ts`가
그 부재를 계속 단언한다.
...
- **개발 서버 화면 확인 완료** (375×812, `dev-테스터A`, Playwright로 직접 조작):

| 단계 | 화면 | 카드 높이 |
|---|---|---|
| 오늘 완료(실제 오늘) | 배너 **없음** | 281px |
| d4·d3·d2·d1 | 주황 `⚠️ … (소멸 D-N)` 배너, CTA **아래** | **347px** |
| 소멸(5일 경과) | 배너 **없음**, 연속 `0일` | 281px |

  ⚠️ **347px는 설계 §10의 330px 목표를 17px 넘는다.** 목표의 목적인 "크루 두 행이
  하단 탭 위"는 지켜진다 — 경고가 뜬 상태에서도 크루 카드 하단 697px < 탭 754px.
  여유가 57px뿐이므로 이 카드에 무엇을 더 얹기 전에 다시 재야 한다.
  ⚠️ 뜨는 쪽을 보려고 **운영 기록을 건드리지 않았다.** 브라우저 시계만 하루씩
  앞으로 돌려 A의 마지막 운동일(08-21)을 어제~5일 전으로 만들어 봤다(홈은 읽기 전용).
- 콘솔 오류 0건
