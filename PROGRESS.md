...
## 🟡 2026-08-21 구현 완료·배포 대기 — 홈 `나의 오늘` + `오늘의 크루`

홈 최상단을 **내 카드 + 크루 카드** 두 장의 비교 구역으로 바꿨다. 구현·로컬 검증까지
끝났고 **운영 배포는 하지 않았다.**
...
- **B 화면의 알림 벨 배지·목록 문구** — 에이전트가 비밀번호를 입력하지 않는다.
  다른 브라우저/프로필로 `dev-fixture-b@gnd.local` 로그인해 확인해야 한다.
- **`전체 크루 보기 ›`** — A의 크루가 2명뿐이라 조건상 안 뜬다. 테스트로만 확인.
