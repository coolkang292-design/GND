...
## 🚧 2026-07-31 — 버그 신고 파이프라인 0052 (DB 적용 ✅ · 실측 ✅ · **배포 전**)

설계 `docs/superpowers/specs/2026-07-31-bug-report-pipeline-design.md`. 브랜치 `feat/bug-report-pipeline`. **마이그레이션 0052 운영 적용 완료 · 배포 안 함 · 릴리스 알림 안 보냄.**
...
- **남은 것**: ① 화면 확인 — 내 정보 톱니에서 신고 시트가 뜨고 제출되는지 ② **폰에 푸시가 실제로 도착하는지**(사장님만 확인 가능) ③ `main` 머지 → 배포. **배포 전에는 폰에 신고 버튼이 없다**
