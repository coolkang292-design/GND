...
- **결정 변경 (2026-07-17, 사용자)**: ① KPI 입력은 "하루량 × 주 N일 → 기간 총량 자동계산"이 기본(총량 직접 입력 토글 유지, `cdac252`). ② **volume(총볼륨)은 챌린지 목표 선택지에서 제외** — 부위·종목별 중량이 달라 기간 목표로 감 잡기 어려움(계획서 §7 "volume 포함" 결정 변경). DB·점수 산식·과거 데이터 렌더링은 유지, UI 선택지만 제거. 웨이트 유저 추천 = 운동 시간·횟수.
