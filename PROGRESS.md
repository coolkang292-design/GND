...
## ⏸️ 2026-08-21 다음 작업 — 홈 스트릭 소멸 경고 복원

홈 개편에서 `StreakCard`를 빼면서 **소멸 경고 배너**(`⚠️ … 소멸 D-N`)가 같이
사라졌다. 사용자가 배포 후 화면을 보고 지적했고, 되살릴 범위를 **경고 배너 하나**로
확정했다(7일 점·헤더 한 줄·옛 카드는 되살리지 않는다).

- 구현 안 함. **다음 세션이 이어서 한다.**
- 계획·함정·검증 절차: `docs/superpowers/HANDOFF-2026-08-21-home-streak-warning.md`
- DB·마이그레이션: 없음
