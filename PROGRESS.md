...
## 🟡 2026-07-31 — 폰 초대 링크 온보딩 수정 (로컬 검증 완료 · 재배포 대기)

폰에서 챌린지 초대 링크를 열었을 때 **일반 크루 선택 화면**과 **초대 코드 입력 화면**이 나온다는 신고를 확인했다. 모바일에서는 챌린지 화면이 초대 코드를 기억하기 전에 온보딩 화면으로 먼저 이동할 수 있었던 것이 원인이다.
...
- 로컬 검증: 관련 테스트 **6/6**, 전체 **711/711(67파일)**, lint·typecheck·build·`git diff --check` 통과
- **운영 서비스에는 아직 재배포하지 않았다.** 사용자 승인 후 검증한 로컬 저장소 기준으로만 Vercel 배포한다. GitHub 배포·푸시는 하지 않는다
- 현재 `setup` 상태의 발급된 초대 코드가 없어 실제 성공 참가 1회는 재배포 후 새 초대 링크로 폰에서 확인한다
