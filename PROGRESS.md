...
- **[미확인] 폰 확인 2건** — ① 운동 추가 → '스쿼트' 검색 → **맨몸 스쿼트** 노출 ② 홈 챌린지 성과 카드가 `0/5일`이 아니라 실제 연속일 표시(오늘 운동 전이어도)
- **[대기] 스칼레또 안내 알림** — `scripts/notify-scarlet-bodyweight.mjs` (DRY RUN 확인 완료, `--send`는 폰 확인 후 사용자 지시로)
