-- 0070: 고강도 인터벌을 공식 프로그램으로 받는다 (설계 2026-08-12 §5)
-- 적용: 사용자가 Supabase SQL Editor에서 이 파일 전체를 한 번 Run한다.
--
-- 왜: 프로그램 회차는 **근력 모양**이고 인터벌 회차는 **시간 모양**이다.
--     0066이 근력 모양을 강제하고 0069가 그대로 이어받아서, 인터벌 계획은
--     지금 서버가 거절한다.
--
--       jsonb_array_length(exercises)  근력 5~6개  ↔  인터벌 4개
--       prescription.restSeconds       60~300 필수 ↔  인터벌 10초(음원이 정함)
--       level_at_start                 2단계       ↔  인터벌 3단계
--
-- ⚠️ 적용된 0066~0069는 고치지 않는다. 함수를 여기서 통째로 다시 만든다.
--    본문은 `docs/db-current-schema.sql`의 현행 정의(0069 적용 뒤 실측)를
--    기계적으로 옮기고 아래 다섯 곳만 바꿨다.
--
-- 바뀐 것
--   ① level_at_start   — 'moderate' 추가 (테이블 check + RPC 검증)
--   ② tabata_minutes   — 계획 payload에서 받는다. null 또는 4·8·16
--   ③ 인터벌 분기      — tabata_minutes가 있으면 종목 정확히 4개 · 각 세트 1개 ·
--                        prescription 없어도 통과. 없으면 기존 근력 검증 그대로
--   ④ 섞임 금지        — 한 등록의 18회가 전부 인터벌이거나 전부 근력이어야 한다
--   ⑤ insert           — workout_plans에 tabata_minutes 열을 싣는다
--
-- 바뀌지 않은 것: 서로 다른 요일 3개, 18회, 주차·회차 범위, 날짜 검증,
--                 소유권·권한, 원자성, 근력 5종의 동작.
--
-- ⚠️ 기존 등록 행은 건드리지 않는다. 'beginner'·'experienced'는 그대로 유효하다.
--
-- ⚠️ 배포 순서: 이 SQL을 먼저 Run하고 앱을 배포한다. 반대로 하면 사용자가
--    인터벌 프로그램을 고를 수 있는데 서버가 등록을 거절한다.

begin;

-- ① 난이도 3단계 — 기존 행(beginner·experienced)은 그대로 통과한다
alter table public.program_enrollments
  drop constraint if exists program_enrollments_level_at_start_check;
alter table public.program_enrollments
  add constraint program_enrollments_level_at_start_check
...
-- 권한은 0066·0069와 동일하게 다시 못 박는다 — create or replace는 기존 권한을
...
-- ── 적용 확인 (Run 뒤 따로 실행) ────────────────────────────
--
-- 1) 난이도 3단계가 들어갔는가 → moderate가 보여야 한다
...
--    and c.conname = 'program_enrollments_level_at_start_check';
--
-- 2) 인터벌 분기가 함수에 들어갔는가 → 1행이어야 한다
-- select p.proname
--   from pg_proc p join pg_namespace n on n.oid = p.pronamespace
--  where n.nspname = 'public'
--    and p.proname = 'create_program_enrollment'
--    and pg_get_functiondef(p.oid) like '%program_invalid_tabata_minutes%';
--
-- 3) 권한이 그대로인가 → authenticated=true, anon=false
-- select p.proname,
