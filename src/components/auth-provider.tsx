 ...
 *
 * ⚠️ `/privacy` (2026-09-03 외부 파일럿): 개인정보 처리방침은 **로그인하지 않은
 *    사람도 읽을 수 있어야 한다.** 여기서 익명 계정을 발급하면 **방침을 읽었다는
 *    이유로 계정이 생긴다** — 그 문서가 약속하는 내용과 정면으로 어긋나고, 운영
 *    DB에 프로필 없는 유령 계정이 쌓인다. 이 화면은 세션이 필요 없는 정적
 *    문서라 `userId=null`인 채로 끝나는 것이 **정상이자 의도**다.
 *
 * ⚠️ 여기에 경로를 더할 때는 그 화면이 **스스로 세션을 만들거나 되찾는지** 확인해라.
 *    아무도 세션을 안 만드는 화면을 넣으면 그 화면은 영원히 `userId=null`이다.
 *    (`/privacy`는 그래도 되는 화면이라 예외다 — 위 문단 참고.)
 */
const NO_ANON_ROUTES = ["/login", "/auth/callback", "/privacy"];
