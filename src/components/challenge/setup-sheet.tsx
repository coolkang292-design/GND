...
export function ChallengeSetupSheet({
  ...
  const [goals, setGoals] = useState<GoalDraft[]>(defaults.goals);
  ...
  function addGoal() {
    const unused = GOAL_TYPES.find((t) => !goals.some((g) => g.type === t));
    setGoals((gs) => [
      ...gs,
      {
        type: unused ?? "frequency",
        target: GOAL_TYPE_META[unused ?? "frequency"].defaultTarget,
      },
    ]);
  }

  function removeGoal(i: number) {
    if (goals.length > 1) setGoals((gs) => gs.filter((_, idx) => idx !== i));
  }

  function loadPrev() {
    if (!prevGoals || prevGoals.length === 0) return;
    setGoals(prevGoals.map((g) => ({ ...g })));
              ...
              <div key={i} className="mt-2 flex items-end gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateGoal(i, {
                        ...
                        target:
                          GOAL_TYPE_META[e.target.value as GoalType]
                            .defaultTarget,
                      })
                    }
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    key={`${i}-${g.type}`}
                    onChange={(e) =>
                      updateGoal(i, { target: parseFloat(e.target.value) || 0 })
                    }
                    className="mt-1 h-11 w-full rounded-card-sm border border-line bg-surface px-3 text-right font-mono text-sm font-bold"
                  />
                </div>
                <button
