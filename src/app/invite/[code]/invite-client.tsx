 ...
 *
 * ⚠️ 2026-09-20: 이 본문이 `page.tsx`에서 여기로 옮겨 왔다. **`page.tsx`가
 *    서버 컴포넌트여야 `generateMetadata`로 카카오톡 공유 카드를 만들 수 있고,
 *    서버 컴포넌트는 `"use client"`를 같이 둘 수 없기 때문이다.** 동작은 한 줄도
 *    바뀌지 않았다 — `params` Promise를 `use()`로 풀던 것이 서버에서 풀린
 *    `code` 문자열을 받는 것으로만 바뀌었다.
 */
export function InviteClient({ code: rawCode }: { code: string }) {
