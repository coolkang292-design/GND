...
export function buildFriendRows(input: {
  ...
  const rows = input.crew.map((member) => {
    const activity = input.activity.get(member.id) ?? EMPTY_ACTIVITY;
    const badge = input.badges.get(member.id);
    ...
    return {
      ...
      status: friendStatus(activity.workedOutToday, input.activeUserIds.has(member.id)),
    };
  });
