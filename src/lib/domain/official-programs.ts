...
export const PULLUP_LADDER_PROGRAM: LadderProgram = {
  ...
  /*
    ⚠️ **`key`의 18과 헷갈리지 마라.** `key`는 DB에 박힌 식별자라 바꾸면 이미
       등록된 사람의 `program_enrollments` 행이 고아가 된다 — 그래서 그것만
       옛 이름으로 남는다. 사람이 읽는 문구는 전부 **24회**다.

    ⚠️ 2026-09-04 오후에 이 두 줄(`title`·`description`)이 "18회"인 채로
       운영에 나가 있었다. 같은 화면 바로 아래에 "총 24회"가 찍혀서 카드가
       스스로 모순됐다. 화면을 열어 봐야 보였다 — `ladder-program.test.tsx`의
       "옛 18회 문구가 없다" 테스트는 **설정 화면만** 렌더해서 못 잡았다.
       `official-programs.test.ts`가 이제 데이터 자체를 훑는다.
  */
  title: "최대 개수를 올리는 24회 사다리",
  description:
    "파벨 차졸린의 러시안 파이터 풀업 루틴이에요. 지금 할 수 있는 최대 개수를 넣으면 5·4·3·2·1로 시작하는 사다리를 만들고, 회차마다 뒤쪽 세트를 1회씩 올려 24회를 미리 계획해요.",
