 ...
 *
 * 한 블록 = **13초 준비 + 8라운드 × 30초**이고 파일은 250.02초다 — 마지막
 * 휴식이 끝에서 잘린다. 8·16분은 이 블록을 2·4번 이어 붙인 것이다.
 *
...
/**
 * 블록 시작의 준비 구간 — 음원이 "3 · 2 · 1"을 부른 뒤 첫 운동이 시작된다.
 *
 * ⚠️ **10이 아니라 13이다** (2026-08-13 실측 + 사용자 확인).
 *
 *    처음엔 파일 길이만 보고 10초로 뒀다 — 250.02초 = 10 + 8×30이 딱 맞아서다.
 *    그런데 화면이 음악보다 빨랐다. 중앙 채널(음악은 좌우로 퍼지고 멘트는
 *    가운데 있다)만 뽑아 재 보니 멘트가 이렇게 깔려 있었다:
 *
 *      10.6 11.4 12.3 13.1 14.0 14.4   ← 첫 카운트다운
 *      30.9 32.0 32.9 34.1             ← 약 20초 뒤 (운동 → 휴식)
 *      41.1 42.2 43.1 44.3 44.9        ← 약 10초 뒤 (휴식 → 운동)
 *
 *    20초·10초 교대는 맞고 **시작점만 3초쯤 늦다.** 그래서 마지막 휴식이
 *    파일 끝에서 잘린다(13 + 8×30 = 253 > 250.02) — 운동 구간은 전부 들어간다.
 *
 * ⚠️ 이 값은 **귀로 맞추는 상수다.** 실측 적합도는 13~15초가 비슷하게 좋았다.
 *    화면이 아직 이르면 1초씩 올려라. 음원을 바꾸면 다시 재야 한다.
 */
export const INTERVAL_PREP_SECONDS = 13;
...
/**
 * 한 블록의 길이 (초) = **음원 파일의 길이**.
 *
 * ⚠️ 준비 구간을 더한 값이 아니다. 4분 음원이 250.02초이고 8·16분은 이것을
 *    2·4번 이어 붙였다. 준비(13초)를 더하면 253이 되는데, 마지막 휴식이
 *    파일 끝에서 잘리기 때문에 실제 한 블록은 250초다. 이 값으로 블록을
 *    나눠야 두 번째 블록의 시작이 음원과 맞는다.
 */
export const INTERVAL_BLOCK_SECONDS = 250;
