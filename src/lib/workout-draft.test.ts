...
describe("loadDraft 예정표 버전 호환", () => {
  it("기존 version 1 임시 운동을 version 7로 보존한다", () => {
  ...
  it("기존 version 2 임시 운동을 version 7로 보존한다", () => {
    vi.stubGlobal("localStorage", {
      getItem: () =>
        JSON.stringify({
          version: 2,
  ...
  it("기존 version 3 임시 운동을 version 7로 보존한다", () => {
    vi.stubGlobal("localStorage", {
      getItem: () =>
        JSON.stringify({
          version: 3,
  ...
  it("기존 version 4 임시 운동을 무동작 필드만 채워 version 7로 올린다", () => {
    vi.stubGlobal("localStorage", {
      getItem: () =>
        JSON.stringify({
          version: 4,
  ...
  it("version 5 임시 운동을 프로그램 필드만 채워 version 7로 올린다", () => {
    vi.stubGlobal("localStorage", {
      getItem: () =>
        JSON.stringify({
          version: 5,
    ...
    expect(draft.suggestedForDayKey).toBeNull();
  });
 ...
 */
describe("loadDraft 프로그램 운동 (version 6 → 7 승격)", () => {
  it("프로그램 메타·처방·세트 피드백을 그대로 복구한다", () => {
    ...
    // v6에는 없던 칸이다. 승격하면서 비어 있어야 한다 — 값이 들어가면
    // 사용자가 직접 담은 운동이 다음 날 아침에 제안으로 오인돼 지워진다.
    expect(draft.suggestedForDayKey).toBeNull();
  });

  /**
   * 이미 v7인 draft는 스탬프를 **그대로** 들고 와야 한다. 여기서 잃으면 제안이
   * 사용자 것으로 승격돼 자정을 넘겨도 안 지워진다.
   */
  it("version 7 draft의 제안 스탬프를 그대로 복구한다", () => {
    vi.stubGlobal("localStorage", {
      getItem: () =>
        JSON.stringify({
          version: 7,
          ...
          effortMessage: null,
          restSeconds: 90,
          exercises: [{ key: "v7", name: "걷기", sets: [] }],
          pausedSeconds: 0,
          pausedAtMs: null,
          lastActivityMs: null,
          tabataMinutes: null,
          program: null,
          suggestedForDayKey: "2026-08-16",
        }),
    });

    const draft = loadDraft("user-id");
    expect(draft.version).toBe(7);
    expect(draft.suggestedForDayKey).toBe("2026-08-16");
  });
...
describe("emptyDraft·newSet 기본값", () => {
  it("새 세션은 program null, 세트 피드백 null로 시작한다", async () => {
    ...
    expect(emptyDraft().version).toBe(7);
    expect(emptyDraft().program).toBeNull();
    expect(emptyDraft().suggestedForDayKey).toBeNull();
